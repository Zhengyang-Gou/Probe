...
def test_direct_successor_link_skips_path_recalculation() -> None:
    ...
    assert result.status is TraversalStatus.RUNNING
    assert probe.next_telemetry_node == 1
    assert result.path == [0, 1]
    assert result.next_hop == 1


def test_down_direct_successor_link_replans_to_same_target() -> None:
    topology = Topology(nodes={0, 1, 2}, edges={(0, 1), (0, 2), (2, 1)})
    table = DelayTable.from_constant_delay(topology, period_slots=1, delay=1.0)
    engine = AdaptiveTraversalEngine(
        topology,
...
def test_relay_node_does_not_record_telemetry_or_advance_target() -> None:
    ...
    assert result.status is TraversalStatus.RUNNING
    assert result.next_hop == 2
    assert probe.next_telemetry_node == 2
    assert probe.visited == {0}
    assert probe.telemetry_record == []
...
def test_link_recovery_up_rejoins_estimated_topology() -> None:
    ...
    probe = ProbeState(
        root=0,
        current_node=0,
        next_telemetry_node=0,
        link_obs_table=obs,
        last_slot=table.slot_at(1.0),
    )
...
def test_finishes_after_all_nodes_are_visited_and_probe_returns_root() -> None:
    ...
    first = engine.on_probe_arrival(
        probe, 0, now=0.0, physical_link_state_provider=up_provider
    )
    second = engine.on_probe_arrival(
        probe, first.next_hop or 1, now=1.0, physical_link_state_provider=up_provider
    )
    third = engine.on_probe_arrival(
        probe, second.next_hop or 0, now=2.0, physical_link_state_provider=up_provider
    )
